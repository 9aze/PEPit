...
def wc_EF21(L, mu, epsilon, n, verbose=1):
    """
    ...
    **References**: The detailed analyses can be found in [1, 2].
    ...
    dx0=problem.set_initial_point()
    compr0=problem.set_initial_point()
    gx0, fx0 = func.oracle(x0)
 

    # Set the initial constraint that is the distance between f0 and f_*
    
    problem.set_initial_condition(func(x0) - fs +coeff*(dx0-gx0)**2 <= 1)
    ...
    for i in range(n):
        ...
        func.add_constraint((compr-gx+dx) ** 2 - epsilon ** 2 * ((gx-dx) ** 2) <= 0)
        dx=dx+compr
    ...
    problem.set_performance_metric(func(x) - fs +coeff*(dx-gx)**2)
...
if __name__ == "__main__":
    pepit_tau, theoretical_tau = wc_EF21(L=1, mu=.1, epsilon=.1, n=1, verbose=1)
